...
function setMicState(state, context) {
  if (!VALID_STATES.includes(state)) throw new Error(`Invalid mic state: ${state}`);
...
async function startListening() {
  if (!micStream) {
    renderMessage('assistant', "I couldn't access your microphone. You can still type your question below.");
...
async function stopListening() {
  ...
  try {
    ...
    if (!userText || !userText.trim()) {
      renderMessage('assistant', "I didn't catch that. Could you say it again?");
